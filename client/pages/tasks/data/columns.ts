...
export const columns: Column[] = [
    ...
    },
    {
        ...
        render: (row: Task) =>
            h(UBadge, {
                color:
                    row.status ===
                    (TaskStatus.TODO as Maybe<TaskStatus> | undefined)
                        ? "gray"
                        : row.status ===
                            (TaskStatus.DONE as Maybe<TaskStatus> | undefined)
                          ? "emerald"
                          : "yellow",
                label:
                    row.status ===
    ...
    },
    {
        ...
        render: ({ updatedBy }) => {
            const auth = useAuthStore();
            const isCurrentUser = updatedBy?.id === auth.user?.id;
            return h(
                "div",
                isCurrentUser ? "Me" : (updatedBy?.name ?? "Unknown"),
            );
        },
        sortable: true,
    },
